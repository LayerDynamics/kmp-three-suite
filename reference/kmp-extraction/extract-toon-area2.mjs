...
try {
  ...
  console.log('File size:', mtlBuf.length)
  ...
  console.log('Param section:', paramStart, 'to', mtlBuf.length)
  console.log()

  // Dump from paramStart to end
  const start = paramStart
  const end = mtlBuf.length
  ...
  for (let i = start; i < end; i += 32) {
    ...
    console.log(`${offset.toString().padStart(4)}: ${hex.join(' ').padEnd(96)} |${ascii.join('')}|`)
  }
} finally {
